...
function getSqlClient() {
    if (!sqlClient) {
        ...
        const connectionString = normalizeUrl(process.env.DATABASE_URL || process.env.POSTGRES_URL);
        
        // SAFETY GUARD: Strict environment validation
        const dbEnv = process.env.DATABASE_ENV;
        if (!dbEnv) {
            throw new Error("FATAL: DATABASE_ENV environment variable is missing. Must be explicitly set to 'development' or 'production'.");
        }

        if (dbEnv === 'development') {
            const devUrl = normalizeUrl(process.env.DEV_DATABASE_URL);
            if (!devUrl) {
                throw new Error("FATAL: DEV_DATABASE_URL is missing. In development, you must explicitly define the permitted development database URL.");
            }
            if (connectionString !== devUrl) {
                throw new Error("FATAL: DATABASE_URL does not match DEV_DATABASE_URL. Refusing to connect to a potential production database in development.");
            }
        } else if (dbEnv !== 'production') {
            throw new Error(`FATAL: Invalid DATABASE_ENV '${dbEnv}'. Must be 'development' or 'production'.`);
        }
