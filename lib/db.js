...
function getPool() {
    if (!pool) {
        ...
        pool.on('error', (err) => {
            ...
            // Reset pool on critical connection error so subsequent requests can reconnect cleanly
            if (pool) {
                pool.end().catch(() => {});
                pool = null;
            }
...
// Helper for single query execution with automatic retry on dead pool
async function query(text, params) {
    const p = getPool();

    // Background schema initialization notice (non-blocking)
    if (!schemaInitialized) {
        schemaInitialized = true;
        initSchemaInternal(p).catch((e) => {
    ...
    try {
        ...
        // If connection terminated or timed out, reset pool to force fresh connection on next request
        if (error.code === 'ECONNRESET' || error.code === 'ECONNREFUSED' || error.message.includes('timeout') || error.message.includes('Connection terminated')) {
            if (pool) {
                pool.end().catch(() => {});
                pool = null;
            }
