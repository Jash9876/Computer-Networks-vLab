...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // If observationCount >= 3 or milestones >= reqCount, mark simulation complete
        const isSimComplete = milestones.length >= reqCount || totalDbEvents >= 3 || rawProg.status === 'completed';
        const bestScore = certRes.rows[0] ? Number(certRes.rows[0].final_score) : (quizRes.rows[0] && quizRes.rows[0].total_questions > 0 ? Math.round((quizRes.rows[0].score / quizRes.rows[0].total_questions) * 100) : 0);
        const isVivaPassed = bestScore >= 70;
        ...
        return res.status(200).json({
            ...
            progress: {
                ...
                progress_percentage: isAcademicComplete ? 100 : (isSimComplete ? 85 : Math.min(85, Math.round((milestones.length / reqCount) * 85))),
