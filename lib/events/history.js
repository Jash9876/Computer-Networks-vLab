...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        function resolveMilestone(expId, stage, eventType, payload) {
            ...
            switch (expId) {
                ...
                case 10: {
                    const actLower = act.toLowerCase();
                    const stgLower = stg.toLowerCase();
                    if (stgLower.includes('10_topology') || (stgLower.includes('topology') && stgLower.includes('10')) || (eventType === 'TOPOLOGY_VALIDATED' && stgLower.includes('10')) || actLower.includes('topology builder') || actLower.includes('verified connections')) res.push('10_TOPOLOGY_COMPLETE');
                    if (stgLower.includes('10_ip') || (stgLower.includes('addressing') && stgLower.includes('10')) || (eventType === 'ADDRESSING_MATCHED' && stgLower.includes('10')) || actLower.includes('all ip addresses verified')) res.push('10_IP_CONFIGURED');
                    ...
                    if (stgLower.includes('10_bgp_prefix') || actLower.includes('network') && actLower.includes('mask') || actLower.includes('advertised') || actLower.includes('prefix advertisement')) res.push('10_BGP_PREFIX_ADVERTISED');
                    if (stgLower.includes('10_bgp_withdrawal') || actLower.includes('withdrawal') || actLower.includes('flap') || actLower.includes('link down') || actLower.includes('fault injection')) res.push('10_BGP_WITHDRAWAL_TESTED');
                    if (stgLower.includes('10_connectivity') || (eventType === 'PING_SUCCESS' && stgLower.includes('10')) || (actLower.includes('ping') && (stgLower.includes('10') || actLower.includes('192.168.20.3') || actLower.includes('192.168.10.2')))) res.push('10_CONNECTIVITY_VERIFIED');
                    break;
                }
            }
        ...
        const expTotalReq = { 1: 3, 2: 4, 3: 4, 4: 9, 5: 5, 6: 5, 7: 10, 8: 8, 9: 8, 10: 6 };
        ...
        return res.status(200).json({
            ...
            progress: {
                ...rawProg,
                completed_at: rawProg.completed_at || (certificateData ? certificateData.issued_at : (eventsRes.rows.length > 0 ? eventsRes.rows[eventsRes.rows.length - 1].created_at : null)),
