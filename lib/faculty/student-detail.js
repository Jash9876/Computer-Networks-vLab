    ...
    try {
        ...
        const progRes = await query(
            `SELECT 
             ...
             LEFT JOIN (
                 SELECT experiment_id, COUNT(*)::int AS event_count
        ...
        const sanitizedProgress = progRes.rows.map(p => {
            ...
            const isSimComplete = mCount >= reqCount || eventCount >= reqCount;
            ...
            return {
                ...
                progress_percentage: isAcademicComplete ? 100 : (isSimComplete ? 85 : Math.min(85, Math.round((Math.max(mCount, eventCount) / reqCount) * 85))),
                isSimComplete,
                isVivaPassed,
                isAcademicComplete,
