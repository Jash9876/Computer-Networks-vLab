...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (passed) {
            ...
            // Server-authoritative: mark experiment completed in PostgreSQL
            await query(
                `INSERT INTO experiment_progress (student_id, experiment_id, status, progress_percentage, completed_at, last_activity)
                 ...
                 ON CONFLICT (student_id, experiment_id) DO UPDATE
                 SET status = 'completed', progress_percentage = 100, completed_at = COALESCE(experiment_progress.completed_at, NOW()), last_activity = NOW()`,
                [user.id, expNum]
            );
        }
